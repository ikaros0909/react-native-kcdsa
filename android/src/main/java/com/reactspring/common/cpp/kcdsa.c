	...
	unsigned int HASH)
{
	int i, j;
	unsigned int ret;
	unsigned char *bzTmp1 = NULL, *bzTmp2 = NULL;
...
JNIEXPORT jint JNICALL Java_kr_or_kisa_seed_kcdsa_KCDSA_KcdsaGenerateKeyPair
	...
	unsigned int* p_data_ = (unsigned int*)(*env)->GetIntArrayElements(env, p_data, 0);
	unsigned int* q_data_ = (unsigned int*)(*env)->GetIntArrayElements(env, q_data, 0);
	unsigned int* g_data_ = (unsigned int*)(*env)->GetIntArrayElements(env, g_data, 0);
	unsigned int* x_data_ = (unsigned int*)(*env)->GetIntArrayElements(env, x_data, 0);
	unsigned int* y_data_ = (unsigned int*)(*env)->GetIntArrayElements(env, y_data, 0);
	unsigned int* kcdsa_param1_ = (unsigned int*)(*env)->GetIntArrayElements(env, kcdsa_param1, 0);
	unsigned int* kcdsa_param2_ = (unsigned int*)(*env)->GetIntArrayElements(env, kcdsa_param2, 0);
	unsigned char* pbSrc_ = (unsigned char*)(*env)->GetByteArrayElements(env, pbSrc, 0);
	int result;
	KISA_KCDSA *kcdsa;
	...
	if(x_data_[0] == 0)
	{
		kcdsa->KCDSA_P->pData = p_data_;
		kcdsa->KCDSA_P->Length = kcdsa_param1_[0];
		kcdsa->KCDSA_Q->pData = q_data_;
		kcdsa->KCDSA_Q->Length = kcdsa_param1_[1];
		kcdsa->KCDSA_G->pData = g_data_;
		kcdsa->KCDSA_G->Length = kcdsa_param1_[2];
	}
	else
	{
		kcdsa->KCDSA_P->pData = p_data_;
		...
		kcdsa->KCDSA_G->pData = g_data_;
		kcdsa->KCDSA_G->Length = kcdsa_param1_[2];
		kcdsa->KCDSA_x->pData = x_data_;
		kcdsa->KCDSA_x->Length = kcdsa_param1_[3];
	}
	
	result = KISA_KCDSA_GenerateKeyPair(kcdsa, pbSrc_, (unsigned int)dSrcByteLen, (unsigned int)qlen, (unsigned int)HASH);
	if(result != 0) return result;

	if(x_data_[0] == 0)
	{
		memcpy(x_data_, kcdsa->KCDSA_x->pData, sizeof(unsigned int) * kcdsa->KCDSA_x->Length);
		memcpy(y_data_, kcdsa->KCDSA_y->pData, sizeof(unsigned int) * kcdsa->KCDSA_y->Length);
		
		kcdsa_param2_[0] = kcdsa->KCDSA_x->Length;
		kcdsa_param2_[1] = kcdsa->KCDSA_y->Length;
		
		(*env)->ReleaseIntArrayElements(env, x_data, x_data_, 0);
		(*env)->ReleaseIntArrayElements(env, y_data, y_data_, 0);
		(*env)->ReleaseIntArrayElements(env, kcdsa_param2, kcdsa_param2_, 0);
	}
	else
	{
		memcpy(y_data_, kcdsa->KCDSA_y->pData, sizeof(unsigned int) * kcdsa->KCDSA_y->Length);
		
		kcdsa_param2_[0] = kcdsa->KCDSA_y->Length;
...
JNIEXPORT jint JNICALL Java_kr_or_kisa_seed_kcdsa_KCDSA_KcdsaSign
        (JNIEnv *env, jobject thiz, jintArray p_data, jintArray q_data, jintArray g_data, jintArray x_data, jintArray kcdsa_param1, jintArray kcdsa_param2, jbyteArray MsgDigest, jint MsgDigestLen, jbyteArray Signature, jint HASH, jbyteArray t_omgri, jint omgri_len)
{
    unsigned int* p_data_ = (unsigned int*)(*env)->GetIntArrayElements(env, p_data, 0);
    unsigned int* q_data_ = (unsigned int*)(*env)->GetIntArrayElements(env, q_data, 0);
    unsigned int* g_data_ = (unsigned int*)(*env)->GetIntArrayElements(env, g_data, 0);
    ...
    unsigned int* kcdsa_param1_ = (unsigned int*)(*env)->GetIntArrayElements(env, kcdsa_param1, 0);
    unsigned int* kcdsa_param2_ = (unsigned int*)(*env)->GetIntArrayElements(env, kcdsa_param2, 0);
    unsigned char* MsgDigest_ = (unsigned char*)(*env)->GetByteArrayElements(env, MsgDigest, 0);
    unsigned char* Signature_ = (unsigned char*)(*env)->GetByteArrayElements(env, Signature, 0);
    unsigned char* t_omgri_ = (unsigned char*)(*env)->GetByteArrayElements(env, t_omgri, 0);
    int result;
    unsigned int SigLen;
    KISA_KCDSA *kcdsa;
    KISA_KCDSA_CreateObject(&kcdsa);
    ...
    result = KISA_KCDSA_sign(kcdsa, MsgDigest_, (unsigned int)MsgDigestLen, Signature_, &SigLen, (unsigned int)HASH, t_omgri_, (unsigned int)omgri_len);
    if(result != 0 ) return result;

    kcdsa_param2_[0] = SigLen;

    (*env)->ReleaseByteArrayElements(env, Signature, Signature_, 0);
    (*env)->ReleaseIntArrayElements(env, kcdsa_param2, kcdsa_param2_, 0);
	...
	unsigned int		omgri_len)
{
	...
	if (Signature == NULL)	return CTR_INVALID_POINTER; // 사인이 NULL이면 오류
		...
		(JNIEnv *env, jobject thiz, jintArray p_data, jintArray q_data, jintArray g_data, jintArray y_data, jintArray kcdsa_param, jbyteArray MsgDigest, jint MsgDigestLen, jbyteArray Signature, jint SignLen, jint HASH)
{
	...
	unsigned int* g_data_ = (unsigned int*)(*env)->GetIntArrayElements(env, g_data, 0);
	unsigned int* y_data_ = (unsigned int*)(*env)->GetIntArrayElements(env, y_data, 0);
	unsigned int* kcdsa_param_ = (unsigned int*)(*env)->GetIntArrayElements(env, kcdsa_param, 0);
	unsigned char* MsgDigest_ = (unsigned char*)(*env)->GetByteArrayElements(env, MsgDigest, 0);
	unsigned char* Signature_ = (unsigned char*)(*env)->GetByteArrayElements(env, Signature, 0);
	int result;
	KISA_KCDSA *kcdsa;
	...
	kcdsa->KCDSA_P->pData = p_data_;
	kcdsa->KCDSA_P->Length = kcdsa_param_[0];
	kcdsa->KCDSA_Q->pData = q_data_;
	kcdsa->KCDSA_Q->Length = kcdsa_param_[1];
	kcdsa->KCDSA_G->pData = g_data_;
	kcdsa->KCDSA_G->Length = kcdsa_param_[2];
	kcdsa->KCDSA_y->pData = y_data_;
	kcdsa->KCDSA_y->Length = kcdsa_param_[3];

	result = KISA_KCDSA_verify(kcdsa, MsgDigest_, (unsigned int)MsgDigestLen, Signature_, (unsigned int)SignLen, (unsigned int)HASH);
		...
		(JNIEnv *env, jobject thiz, jint p_len, jint q_len, jintArray p_data, jintArray q_data, jintArray g_data, jintArray kcdsa_param, jint HASH)
{
	...
	result = KISA_KCDSA_GenerateParameters((unsigned int)p_len, (unsigned int)q_len, kcdsa, (unsigned int)HASH);
	if(result != 0) return result;

	memcpy(p_data_, kcdsa->KCDSA_P->pData, sizeof(unsigned int) * kcdsa->KCDSA_P->Length);
	memcpy(q_data_, kcdsa->KCDSA_Q->pData, sizeof(unsigned int) * kcdsa->KCDSA_Q->Length);
	memcpy(g_data_, kcdsa->KCDSA_G->pData, sizeof(unsigned int) * kcdsa->KCDSA_G->Length);

	kcdsa_param_[0] = kcdsa->KCDSA_P->Length;
	kcdsa_param_[1] = kcdsa->KCDSA_Q->Length;
	kcdsa_param_[2] = kcdsa->KCDSA_G->Length;
	kcdsa_param_[3] = kcdsa->Count;
	kcdsa_param_[4] = kcdsa->SeedLen;

    (*env)->ReleaseIntArrayElements(env, p_data, p_data_, 0);
    (*env)->ReleaseIntArrayElements(env, q_data, q_data_, 0);
    (*env)->ReleaseIntArrayElements(env, g_data, g_data_, 0);
    (*env)->ReleaseIntArrayElements(env, kcdsa_param, kcdsa_param_, 0);
